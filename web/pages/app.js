...
const initial = () => ({
  ...
  ore: 100,
  damage: 0,
  equipment: { drill: 1, cart: 1, lamp: 1 },
});
let state = initial();
...
function render() {
  ...
  document.querySelector("#readout").innerHTML = [
    ...
    `탭: ${format(tapDamage(state.equipment.drill))}`,
    `자동 DPS: ${format(dps)}`,
...
const checks = [
  ...
  ["심도 240m에서 수정 지층 진입", () => unlockedMaximumLevel(240) === 17],
  ["초기 크리티컬 확률은 5%", () => criticalChance(1) === 0.05],
];

const results = checks.map(([name, run]) => ({ name, passed: Boolean(run()) }));
document.querySelector("#checks").innerHTML = results
  .map(({ name, passed }) => `<li class="${passed ? "pass" : "fail"}">${passed ? "PASS" : "FAIL"} · ${name}</li>`)
  .join("");
...
document.querySelector("#metrics").innerHTML = [
  ["초기 탭", tapDamage(1)],
  ["첫 자동 DPS", automationDamagePerSecond(2)],
  ["첫 정련", `×${refinementMultiplier(1)}`],
  ["첫 광차 비용", upgradeCost("cart", 1)],
