...
- **연속 좌표:** `depth(m) → y(pt)` 변환만 쓴다. 4m 경제 세그먼트를 화면 밴드로 만들지 않는다.
- **하강:** 한 세그먼트를 파는 동안 광부·드릴·균열이 4m만큼 아래로 이동한다. 돌파 순간
  지층이 위로 흐르고 작업 위치가 복귀해 카메라가 따라 내려가는 것으로 읽힌다.
- **지표 작업선:** 24m보다 얕을 때 부족한 과거 깊이는 열린 지표 갱도 여백으로 채운다.
  음수 암반을 만들거나 헤드를 화면 상단에 붙이지 않는다.
- **텍스처:** 지역별 320×128 `ShaftRock_*` 와이드 벽면을 큰 지층 덩어리에 맞춰 채운다.
  정사각 암반 조각을 반복·반전하지 않는다. `ShaftGantry`는 현재 작업 리그로 쓰고 광맥층에만
  `SeamVein`을 겹친다. 지표의 빈 공간은 `ShaftSurface` 캐노피가 맡는다.
  일곱 자산은 배경·구조·상태로 역할이 분리된다 (D-048).
- **현재 지반:** 배경 지층과 별개로 화면 폭 대부분을 채우는 현재 `ShaftRock_*`를 작업선에
  한 번 더 둔다. 이 지반만 기본 탭 대상이다. 배경 전체를 투명 제스처로 만들지 않는다.
- **도구:** 광부 몸과 `MiningPickaxe`를 분리한다. 준비 자세에서 손잡이 끝을 축으로 내려찍고
  복귀한다. 몸 전체를 회전시켜 타격을 흉내 내지 않는다.
- **통로 이력:** 지나온 64구간까지 당시 드릴 폭을 유지한다. 레일·광차·램프는 구매 시점 이후
  통로에 실제 설치되어 위쪽에 누적된다.
- **파괴 표현:** 사각 와이프와 방사형 충격 별을 쓰지 않는다. 첫 손상부터 세로 균열을
  위에서 아래로 공개하고 light→medium→heavy로 굵힌다. 파괴 순간 이전 지반을 좌우로 갈라
  아래로 떨어뜨린 다음 카메라가 내려간다.
- **입력 순서:** touch-down에는 지반 눌림과 명암만 즉시 보여주고, release에서 데미지를
  확정한다. 확정 뒤 곡괭이 하강→지반 압축/파편→균열 갱신 순으로 한 인과를 만든다.
...
- **어둠:** 막장 아래는 램프가 닿는 만큼만 보인다. 조도는 석탄 오버레이의 불투명도로만
  표현하고 색을 바꾸지 않는다.
- **눈금:** 장면 위 왼쪽에 겹쳐 20m마다, 굴착 헤드에는 0.1m 단위 심도를 적는다. 눈금용
  별도 열을 두어 갱도를 오른쪽으로 밀지 않는다.
- **명판:** 지역 경계와 최고 기록 위치에만 황동 명판을 단다.
- **약점:** 보이는 표식은 36pt, 실제 조작 영역은 48×48pt다. 탭 대상의 최소 크기를
  장식 크기에 종속시키지 않는다.
- **장비 가시화:** 드릴 티어는 도구·통로 폭·파편, 광차는 레일·대수·속도, 램프는 설치 간격과
  빛 반경, 임팩트는 스윙 강도를 바꾼다. 숫자만 바뀌는 업그레이드는 허용하지 않는다.

### First Rock Onboarding

- 첫 화면은 별도 삽화나 설명 카드가 아니라 실제 `Shaft Column`의 지표 작업면이다.
- 제목→4m 진행 레일→암반→방치형 안내 순서로 한 화면에 놓고 상단 안전영역부터 시작한다.
- 큰 현재 지반이 한 번의 기본 탭 대상이다. 갱도 배경·구조물·장비를 버튼으로 노출하지 않고,
  약점만 그 지반 위의 별도 48×48pt 버튼으로 둔다.
- 타격 데미지와 진행률은 즉시 바뀌고, 파괴 보상과 드릴 강화만 별도 단계로 분리한다.
- 집중 출정과 권한 카피는 첫 암반 화면에 주 동작으로 두지 않는다. 방치가 기본이라는 한 줄만
  남기고, 집중은 홈에서 발견하게 한다.
