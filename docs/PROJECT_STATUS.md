...
업데이트: 2026-08-02 (웹 기준 D-055~D-060 앱 포팅 완료)

## Current state

- 단계: **무한 성장으로 전환.** 장비 레벨 상한을 없애고 정련 등급(6레벨마다 ×2.5)을 두 번째
  곱연산 축으로 추가했다. 지갑은 `BigNumber`이고 곡선은 수렴하지 않는다 — 180일에 라이트
  215km, 헤비 486km. 집중 없이도 같은 심도에 도달한다 (D-069, D-070)
...
## Verification state (2026-08-02 unbounded growth)
...
- ~~`RockEngine`의 512층 절단 손실~~ → D-066에서 해소. `MiningLoop.advance`가 잔여 데미지를
  최대 12패스까지 재적용한다. 6,144층(24,576m)을 넘는 단일 정산은 여전히 절단되지만
  `wasTruncated`로 보고된다
