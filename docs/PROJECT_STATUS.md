...
- Pages PR은 build/smoke만 수행하고 main push만 배포한다. PR #3을 merge commit
  ...
  0건을 확인했다 (D-080).
- 공개판의 광부·막장·균열이 고정 작업선에 남고 320px 암반 위상이 순환하던 문제를 D-081로
  대체했다. 웹은 같은 진행도에서 `headDepth`, `cameraDepth`, `headScreenOffset`을 파생한다.
  첫 65%에는 광부·드릴·막장이 최대 72.8px 실제로 내려가고 이후 카메라가 따라잡는다. 암반
  위상은 modulo 없이 단조 감소해 순환 경계의 역보간도 없다. 기존 광부 스트립·파쇄 립·균열은
  역할에 맞으므로 새 이미지는 추가하지 않았다. 로컬 1280×720과 390×844의 54% 진행에서
  광부가 각각 약 62px·61px 하강했고 모바일 가로 overflow가 없음을 확인했다. commit
  `d36fd9f`의 Pages run `30775979619`가 성공했으며 공개 URL에서도 54% 진행 광부 61px·립
  60px 하강, 후반 암반 위상 `-224→-311→-336px` 단조 이동과 돌파 뒤 연속성을 재확인했다.
  같은 commit의 validation run `30775979660`도 Core·App 단위·unsigned iOS build·웹·에셋·
  Swift 파일 크기 job 전부 성공했다.
...
웹 **20/20**, 장기 진행 에셋 **7/7**, generic iOS device build 통과다. Pages 게임은
