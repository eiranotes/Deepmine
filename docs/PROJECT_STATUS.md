...
업데이트: 2026-08-01 (D-055 Sites v4 배포·클리커 비교 감사)
...
| P4-9 | 첫 뷰포트 탭→보상→강화 폐루프와 공명 결절·SFX 체감 보강 | 제안·미구현 |
...
- 공식 Cookie Clicker 웹판과 비교하면 DeepMine은 공간적으로 누적되는 하강 장면은 더 강하지만,
  ...
  다음 웹 개선은 첫 뷰포트에 현재 층 보상/완료 ETA와 구매 가능한 다음 장비를 붙이고, 기존
  `ResonanceNode`를 간헐 보상 사건으로 올리는 순서다
