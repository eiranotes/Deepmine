...
- 옛 Lv.200 제품 상한은 제거됐고 장비별 정련은 6레벨마다 열린다. 다만 심도 1,499,925m에서
  ...
  `BigNumber`를 보존한다.
- 활성 집중 세션과 홈 자동/오프라인 채굴은 같은 시간 예산을 쓴다. 세션 복구 전, 시작 버튼의
  비동기 준비 구간, 활성 세션 중에는 숨은 홈 타이머와 오프라인 정산을 모두 중지한다.
- 장시간 정산은 첫 512세그먼트 뒤 carry pass당 6,144세그먼트, 최대 12패스를 쓴다.
  150km·500km의 8시간×75% 정산은 `wasTruncated=false`, `unspentDamage=0`이다.
- 자동화 전에는 첫 광차 Lv.2를 ◆180 절대 저축 목표로 유지한다. 광석 134에서도 더 싼
  드릴로 추천을 바꾸지 않으며, 광차 구매는 충분히 모일 때까지 비활성화한다.
- 홈은 자동 광석/초·층/초와 구매 전후 출력/ETA 변화를 보여 주며, 정련은 일반 장비 구매와
  구분된 도약 피드백을 쓴다. 램프는 직접 ×2.5나 고정 ×√2.5가 아니라 실제 전후 치명타
  배율과 기대 탭 출력을 표시한다.
- SwiftData의 일반 저장·원자적 명령·세션 종료 커밋은 현재/기억 장비, 정련 단계와 달성 업적을
  모두 보존한다. 빈 레거시 필드는 현재 장비·정련 없음·업적 없음으로 안전하게 복구한다.
- 5km·20km·100km는 새 경제 지역이 아니라 심연 내부의 pressure/fault/core 시각 지질
...
- Pages PR은 build/smoke만 수행하고 main push만 배포한다. PR #3을 merge commit
  ...
  0건을 확인했다 (D-080).
- D-081의 진행률 연속 하강은 사용자 실화면 판정에서 막장이 미끄러지는 동작으로 읽혀 D-082가
  대체한다. 현재 웹은 한 4m 파티션의 0~99%에서 광부·막장·카메라를 고정하고 절삭 홈·균열만
  위에서 아래로 진행한다. 완파한 순간에만 `brokenLayers` delta를 render-only 낙하 이벤트로
  바꾸며, 한 접촉이 N개를 부수면 `4N m` 한 번의 낙하로 합친다. N=1/2/5/20은 각각
  760/940/1178/1538ms이고 화면 이동은 112/162/228/300px로 제한하되 추가 지층 스크롤과
  `N개 구간 · 4N m` cue로 전체 거리를 보존한다. 활성 낙하 중 새 파괴는 이벤트 배열 대신
  다음 batch 하나에 합산한다. 기존 타격 스트립만 내리면 서 있는 캐릭터가 미끄러져 보여
  ImageGen 참조 생성과 크로마키 제거로 `MinerDescentStrip.png` 384×96 release/fall A/fall B/
  landing 4프레임을 추가했다. 로컬 1280×720과 390×844에서 진행 0→89% 광부 위치 0px,
  camera 8.00m 고정, 완파 중 약 45.36px 낙하와 종료 뒤 12.00m 정착, 모바일 가로 overflow 0을
  확인했다. 웹 22/22와 lint가 통과했다. commit `6980f41`의 Pages run `30779832654`가
  build/deploy 성공했고 공개 1280×720에서도 0→98% 위치 변화 0px, 완파 중 112px 월드·약
  45.36px 광부 낙하, 12.00m 정착과 콘솔 오류 0건을 재확인했다. validation run
  `30779832662`의 Core·App unit·unsigned iOS build·Swift file-size·웹·생성 에셋 job도 모두 성공했다.
- 최신 로컬 D-083은 D-082의 사람 낙하를 다시 대체한다. 앱과 웹의 막장은 케이블·레일에
  체결된 현수식 채굴 리그이며 절삭 중 `worklineDepth=faceDepth`에 고정된다. 숫자상 헤드만
  `faceDepth + progress × 4m`로 진행하고, 완파 뒤 클램프 해제→7~14px 프레임 dip→지층 상승·
  윈치 cue→재체결로 N개 구간을 한 번에 내린다. ImageGen 생성 프레임·드릴 T1/T2/T3·분기
  모듈 6종과 소형/넓은 리브/높은 사이드포드/중장갑 하우징 4종을 앱과 웹에 편입했다. 모든
  장비 레벨은 정확한 D/C/L 레벨판, 4칸 정비 셀 또는 실제 하우징 PNG를 바꾸고 모든 정제는
  R단계 각인판을 바꾸므로 3단계 이후에도 구매 결과가 사라지지 않는다. 매 4레벨 세대는
  `G숫자`뿐 아니라 1~4형 실루엣을 교체한다. 웹의 `rigVisual.ts`가 설치 문구와 asset 선택의
  정본이며 앱은 이를 같은 문자열·상태 계산으로 미러링한다. 앱/웹 표찰은 각각
  9pt/10px, 셀은 5pt/6px로 확대했고 VoiceOver/ARIA는 세 도구의 레벨·티어·세대·셀·R·분기와
  윈치 상태를 함께 읽는다. 브라우저에서 드릴 Lv.1→2가 T1→T2, D1→D2, 셀 0→1로 바뀌었고 절삭
  10%에서 경제 헤드 8.4m·camera 8.00m·리그 작업선 고정을 확인했다. 최신 웹 29/29·lint와
  에셋 14/14, 최신 앱 `build-for-testing` 컴파일이 통과했다. 앱 런타임은 CoreSimulator의
  반복 `Mach -308` 오류로 최신 실행 증거가 없다. 검증된 웹은 사용자의 배포 요청에 따라
  `main` push와 Pages workflow의 D-083 배포 대상으로 전환했다.
  최종 독립 Pro 재감사는 웹 기준 SHIP, P0/P1 구현 이슈 없음으로 판정했고 최신 1280×720·
  390×844 하우징 합성 재캡처만 비차단 후속 확인으로 남겼다.
...
웹 **29/29**, D-083 생성 리그 에셋 **14/14**, 장기 진행 에셋 **7/7**,
최신 generic iOS Simulator `build-for-testing` 통과다. 하우징 추가 전 App focused 24/24와
최신 App 런타임 실행을 혼합하지 않는다. Pages 게임은
...
- 단계: **장기 성장으로 전환.** 옛 Lv.200 제품 상한을 제거하고 정련 등급(6레벨마다 ×2.5)을
  두 번째 곱연산 축으로 추가했다. 지갑은 `BigNumber`이고 검증한 180일 범위에서는 수렴하지
  않는다 — 라이트 215km, 헤비 486km. Lv.100,000 산술 안전 천장 이후는 아직 무상한 계약이
  아니다. 집중 없이도 같은 심도에 도달한다 (D-067, D-069, D-070)
- 단계: **웹 기준 D-055~D-060을 앱에 전량 포팅 완료.** 앱은 Core 경제를 직접 사용하고,
  웹은 같은 공식과 설비 파생을 `coreBalance.ts`에 복제해 패리티 테스트로 감시한다.
  갱도 연출·행동대·공명 결절·타격 변주·설비 누적은 양쪽에 모두 있다 (D-065)
...
| P4-6 | 단일 암반 좌표·고정 막장·무입력 자동 하강 웹 기준안 (D-054) | 완료·고정 막장은 D-081로 대체 |
...
| P4-15 | 웹 설비 파생을 `coreBalance.ts`로 옮기고 패리티로 묶기 (D-065) | 완료 |
| P4-9 | 첫 뷰포트 암반 보상·완료 ETA·추천 강화 폐루프 (D-056) | 웹·앱 완료 |
| P4-10 | 공명 결절·타격 변주·자동 생산의 장면 누적 (D-057, D-058) | 웹·앱 완료. 8-bit SFX는 웹만 |
| P4-11 | 면 단위 충격 전달·작업 데크/광원 내실 단계 (D-060) | 웹·앱 완료 |
| P4-16 | 광부·막장 실제 하강, 후반 카메라 추종, 암반 위상 역보간 제거 (D-081) | 웹·Pages 완료·공개 브라우저 검증 |
| P4-17 | 파티션 완파 뒤 단일·다중 낙하와 전용 착지 스트립 (D-082) | 웹·Pages 완료·공개 브라우저 검증 |
| P4-18 | 고정 작업면 현수식 리그·모든 레벨/정제/분기의 물리 변화 (D-083) | 앱·웹 구현·웹 29/29·빌드 검증, main/Pages 배포 대상 |

## 이전 D-044 진행 곡선 — 역사적 스냅샷

아래는 무한 성장·정련 전환 전의 당시 `DeepMineBalanceCLI --days 180` 실측이다. 최신 곡선은
`docs/BALANCE_REPORT.md` 최상단의 30일 2.7~5.0km, 90일 19.8~55.7km,
180일 215.3~486.4km 결과를 따른다. 탭 입력과 오프라인 8시간 캡이 모두 반영된 값이다.
...
- 클리커 장비(드릴=탭, 광차=자동화, 램프=크리티컬·시야), 비용 1.34, 최고 심도 기반
  다음 레벨 해금 레일과 Lv.100,000 산술 안전 천장·6레벨 정련
...
- 웹 전용 생성 에셋 `ShaftFrontierLip`이 열린 통로와 현재 암반을 같은 U자형 파쇄 경계로
  ...
  전신 접촉·반동 4프레임으로 한 장에 묶어 별도 루프를 제거했다. 두 에셋은 D-062에서 앱
  카탈로그에도 편입했고 `artifacts/imagegen/web-gamefeel-v1/`에 원본·추출·4색 처리·provenance를 보존한다
- 첫 실행을 실제 갱도 장면으로 통합. 탭마다 `mineFace`가 저장되고 첫 4m 파괴 뒤 광석 100과
  수정 1개를 지급하며, 드릴 2레벨 구매 즉시 홈을 연다. 옛 설명 단계 저장도 같은 암반으로 복구
- SwiftUI 막장은 전체 폭의 생성 지반을 실제 `Button`으로 쓴다. 곡괭이를 몸과 분리하던
  이전 표현은 D-062에서 단일 4프레임 액터로 대체했다
...
## Historical verification snapshot (2026-08-02 unbounded growth, superseded)

아래 수치와 미구현 판정은 그 단계의 기록이다. 현재 검증은 문서 최상단과 `BUILD_REPORT.md`의
current-audit closeout을 따른다.

- `swift test --package-path DeepMineCore`: **255/255 통과**, 실패 0
- `DeepMineAppTests`: **134/134 통과**, 실패·skip 0
- generic iOS unsigned build: **통과**
- 밸런스 CLI 30/90/180일 실행. 심도 곡선은 위 표, heavy/light 심도 비 1.82~2.26배
- **미검증**: 이번 전환의 시뮬레이터 육안 확인을 하지 않았다. 앱은 빌드되고 유닛 테스트는
  통과하지만, 큰 수 표기(`1.23e42`)가 실제 화면에서 어떻게 보이는지, 정련 구매 UI가 아직
  없다는 점은 화면으로 확인하지 않았다
- **미구현**: 정련을 앱에서 살 수 있는 UI가 없다. Core 엔진과 저장은 있으나 화면이 없어
  플레이어는 아직 정련을 쓸 수 없다
- **미판정**: 180일 486km가 적절한 속도인지. 게이트는 30일만 본다

## Historical verification snapshot (2026-08-02 web parity and the app port, superseded)

당시 단계의 최종 코드 기준 실측이다. 현재 수치로 사용하지 않는다.
...
- `MiningLoop.advance`는 첫 512 뒤 최대 11개의 6,144 carry window를 써 한 호출에서 최대
  68,096세그먼트를 순회한다. 150/500km 8시간은 완주하지만 더 큰 병적 입력은 종료 보장을
  위해 `wasTruncated`와 `unspentDamage`를 남긴다.
- 지갑과 주요 구매 표면은 `BigNumber`다. 다만 `NextStepPlanner` 진행도와 호환용
  `UpgradeRecommendation.cost: Double`을 외부 호출자가 직접 쓰면 극후반 포화 위험이 남는다.
- 30/90/180일 수식 시뮬레이터는 있지만 실제 홈 추천·저축·정련·MAX 정책을 그대로 구동하는
  UI 정책 시뮬레이터는 없다. 장기 경제 완료 판정은 그 결과까지 보류한다.
- 웹은 JavaScript `number` 기반 formula harness다. iOS의 exponent 383 이상 구매 표면과
  같은 수 범위를 보장하지 않는다.
