...
- [ ] D-055 웹 기준의 단일 암반 좌표·연결 파쇄 경계·전신 타격을 SwiftUI 홈 갱도에 포팅
...
- [ ] 첫 뷰포트 하단에 `현재 층 예상 광석·완료 ETA·구매 가능한 다음 장비`를 한 줄 행동대로 고정
- [ ] 탭 접촉점의 데미지·층 보상·진행률을 한 초점에 모으고, 파쇄 완료 시 광석→장비 비용의
      충당 정도를 즉시 보여 탭→보상→강화 폐루프를 만든다
- [ ] 기존 `ResonanceNode`를 2~5분 간격의 DeepMine 고유 공명 결절 사건으로 연결하고,
      등장·클릭·소멸·배율 보상·놓침 상태를 구현한다
- [ ] 전신 타격의 준비/접촉 타이밍 변주, 크리티컬 포즈·붕괴 변주와 8-bit SFX를 결합해
      820ms 동일 반복의 기계적인 인상을 줄인다
- [ ] 장비 구매 뒤 광차 대수·적재량·광부/설비 누적이 첫 화면에서 즉시 늘어나는 자동 생산 증거를 강화
