...
````text
목표: 90초 안에 루프 1회를 완주시키는 온보딩

흐름:
설명 2장 (차단 개념 / 세션 개념)
→ 90초 데모 세션 (차단 없음, 보상 → 강화까지 실제로 체험)
→ 권한 요청 (FamilyControls → AlarmKit → 알림 순서)
→ 첫 15분 세션 유도

핵심 제약:
- **권한 요청은 데모 이후에 배치한다.** 가치를 먼저 보여준다
- 각 권한 거부 시에도 앱이 끝까지 동작해야 한다
- 데모 세션 보상은 실제 저장소에 반영한다 (버려지는 체험 금지)

완료 기준: 빌드 성공, 권한 3종 각각 거부한 경로가 전부 동작
````
...
| P7~P8 | 온보딩 90초 완주, 권한 거부 경로 3종 동작 |
